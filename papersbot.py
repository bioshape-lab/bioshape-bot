...
def initTwitter():
    if 'CONSUMER_KEY' in os.environ:
        ...
        with open("%s/credentials.yml" % PATH, "r") as f:
...
def readFeedsList():
    with open("%s/feeds.txt" % PATH, "r") as f:
...
class PapersBot:
    ...
    def __init__(self, doTweet=True):
        ...
        try:
            with open("%s/config.yml" % PATH, "r") as f:
